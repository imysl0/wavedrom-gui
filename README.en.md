...
- **Visual editing, written back**: the editor reuses `index.html`; on save the fence content is written back for code blocks, while images get their embedded WaveJSON metadata updated in real time and their pixels redrawn by the editor (flushed 5s after you stop, or immediately when the panel closes or loses focus); **write-backs keep the original style** (`wavedrom-gui.imageExportTheme`, default `auto`: official render / editor vector rebuild / modern render are each redrawn in kind, so editing rounds never wash the style away)
