...
- **Point-and-click + brushes**: 24 waveform states, paint by dragging, drag right to hold, right-click to erase; the brush bar folds through five layouts and supports direct keyboard shortcuts; auto de-glitch is off by default (toggle it in the preview footer or in Settings; same-level runs normalize to "run head + holds"), and the preview footer offers a one-click de-glitch that sweeps the whole diagram and reports how many glitches were removed; cycle-level insert/remove — hover a gap in the time ruler for whole-column edits (on touch, tap a header column to toggle the same capsule), and per-lane edits from the cell popup
...
- **Wide labels, never squeezed**: an over-long data-frame label is elided (start…end character) instead of being compressed, and auto scale (off by default; switch it under Settings → Brushes or Chart → Waveform, with a configurable limit defaulting to 4×) grows the horizontal scale to fit the widest label; importing or first typing a wide label asks once whether to enable it
