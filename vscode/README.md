...
- **原图是哪种导出就按哪种重绘**：编辑器导出菜单里的「WaveDrom 渲染」与「编辑区矢量重建」两种来源都会被识别，保存写回时保持同一种风格（编辑区导出的图不会再被重绘成官方渲染样式）。来源识别：新导出的文件自带标记（SVG metadata 的 `data-export` 属性 / PNG 关键字 `WaveDromGui` 的 iTXt 块），旧 SVG 按结构指纹判断（官方渲染的 `waves_<n>` 特征、编辑区导出的 `data-editor-export` 根属性）；标记与指纹都识别不出来时（如无标记的旧 PNG）按 **skill-modern** 兜底重绘——新工具链默认导出的就是现代风格，识别失败时按它重绘最接近原图。skill 导出的图也带标记：traditional 模式归 wavedrom，modern 模式标记为 `skill-modern`——面板会内联同一套无浏览器渲染器忠实重绘（保持透明底；节点位置等选项按渲染默认值，字体以面板环境为准，与原导出的嵌入字体可能略有差异）
- **PNG** 按原图像素宽对齐重绘（比例 = 原图宽 ÷ 当前自然宽，夹在 1×–4×，原图读不到时退回 2×），避免重绘后 Markdown 里的布局跳动；编辑区来源的底色随当前主题，官方渲染保持白底；**SVG 整文件重绘**（文件里手工做的图形改动会被覆盖）
...
- **`wavedrom-gui.imageExportTheme`** — 编辑波形图片**保存时**像素用哪种风格重绘（内嵌 WaveJSON 无论选哪档都会更新）：
  - `auto`（默认）：**原风格保真**——按图片内嵌标记 / 指纹检测到的导出来源重绘：编辑器导出的图保持编辑器风格，现代渲染的图保持现代，官方渲染的图保持官方传统
  - `modern`：一律按**现代渲染**重绘，无论图片来源
  - `traditional`：一律按 **WaveDrom 官方（传统）**渲染重绘，无论图片来源

  覆盖点在编辑面板打开时生效：改设置后面板会提示重开；已打开的面板保持原来的风格。右键菜单「使用 WaveDrom-Gui 编辑器打开」与 md 预览「编辑波形」两条入口都受它控制。
...
- **图片像素写回**：轮询脚本在图片目标上把当前画面一并渲染随消息上报（按原图来源分派：WaveDrom 渲染 / 编辑区矢量重建 / skill-modern 内联渲染器三选一，识别失败兜底 skill-modern；PNG 走 canvas 光栅化、SVG 直接给矢量文本，`k` 之外新增 `pixels` 消息类型），宿主暂存内存；停手 5s、面板关闭（`onDidDispose`）或切走标签（`onDidChangeViewState`）时一次性原子落盘并调用 `markdown.preview.refresh` 刷新预览。渲染之所以提前到轮询里做，是因为面板关闭时 webview 先被销毁、之后没人能再渲染画面——宿主手里必须始终有最新一份像素，「关闭即写回」才成立
