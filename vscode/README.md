...
- **原图是哪种导出就按哪种重绘**：编辑器导出菜单里的「WaveDrom 渲染」与「编辑区矢量重建」两种来源都会被识别，保存写回时保持同一种风格（编辑区导出的图不会再被重绘成官方渲染样式）。来源识别：新导出的文件自带标记（SVG metadata 的 `data-export` 属性 / PNG 关键字 `WaveDromGui` 的 iTXt 块），旧 SVG 按结构指纹判断（官方渲染的 `waves_<n>` 特征、编辑区导出的 `data-editor-export` 根属性）；旧 PNG 无标记时按 WaveDrom 渲染（历史行为）。skill 导出的图同样按 WaveDrom 渲染
...
- **图片像素写回**：轮询脚本在图片目标上把当前画面一并渲染随消息上报（按原图来源选 WaveDrom 渲染或编辑区矢量重建；PNG 走 canvas 光栅化、SVG 直接给矢量文本，`k` 之外新增 `pixels` 消息类型），宿主暂存内存；停手 5s、面板关闭（`onDidDispose`）或切走标签（`onDidChangeViewState`）时一次性原子落盘并调用 `markdown.preview.refresh` 刷新预览。渲染之所以提前到轮询里做，是因为面板关闭时 webview 先被销毁、之后没人能再渲染画面——宿主手里必须始终有最新一份像素，「关闭即写回」才成立
