...
- **视图**：**设置 →「视图」**里可选择预览在右侧 ⇄ 预览在下方，并独立显示/隐藏实时预览窗口与 WaveJSON 代码窗口；两处**可拖拽分割条**自定义布局（预览区大小、信号列宽，双击重置），偏好持久化。**窗口高度不足 600px（或宽度 ≤800px）时**，自动模式整体切到**简约（手机）布局**：工具栏收紧、左栏收成 ◫ 抽屉、预览/代码收成入口按钮（这也是矮窗口下腾出编辑空间的主要手段）。拉高 / 变宽即回到桌面布局，全程不改偏好
