...
| `P` / `N` | 同 `p` / `n`，另带边沿箭头标记 |
| `u` / `d` | 上升沿（0→1）/ 下降沿（1→0），单次 S 曲线跳变。**没有大写形式**：`U`/`D` 官方无对应砖块，会被画成 `x` 不确定框 |
| `h` / `l` | 高 / 低电平段，跳变处是**垂直沿**（与 `0`/`1` 的斜坡过渡形状不同），可与 `H`/`L` 混搭手工拼时钟，如 `xhlhLHl.` |
| `H` / `L` | 同 `h` / `l`，垂直沿上另带箭头标记 |
| `x` | 不确定 / 无关态（交叉阴影框），多bit数据 “无有效数据” 时推荐使用 |
| `z` | 高阻态 |
...
| `--auto-scale` | `on` \| `off` | `on` | **仅现代模式**：自动缩放——按最宽的 `data` 标签自动抬高 `hscale`，让标签放得下。只增不减：文档里显式写了更大的 `hscale`（如 `config.hscale: 4`）时以其为准，不被压低 |
| `--auto-scale-max` | `1`–`8` | `2` | **仅现代模式**：自动缩放的倍数上限。到上限仍放不下的标签**改为省略显示**（`开头…末尾字符`，绝不做字距压缩），不会把图拉得极宽 |
| `--no-auto-scale` | — | — | 等价于 `--auto-scale off` |
...
- **宽度按真实字形估**：等宽字体下 ASCII ≈ 0.53em，中文/全角正好是它的**两倍**（1.06em）。估算按全角 2 单位、半角 1 单位计，所以中文标签不会被算窄一半（早前按字符数估算会让中文标签显示溢出或压缩过度）。`—`（破折号）与 `…`（省略号）也按全角计。
- **自动缩放默认开**：渲染前扫描所有 `data` 标签，按「标签自然宽度 + 8px 内边距 ÷ 该数据框的格宽」算出需要的档位，把 `hscale` 抬到够用，**上限默认 2×**（`--auto-scale-max` 可调到 1–8）。只增不减，不会压低文档显式设置的更大 `hscale`。
- **到上限仍放不下 → 省略显示**：标签按字符省略为「开头…末尾字符」（如 `一骑红尘妃子笑` → `一骑…笑`），空间再不够就只留开头，始终在数据框宽度内以正常字距显示尽可能多的字符；不再使用 `textLength` 压缩字距（那样会把汉字挤成一团）。
- 想要标签**完整显示**：把 `--auto-scale-max` 调大（例如中文长标签常用 3–4），或在 WaveJSON 里直接写更大的 `config.hscale`。
